...
function syncStartPostSurveyButton() {
    ...
    if (!btn) return; /* button removed — post-survey opens automatically after voting */
    if (!gameState.isAdmin) {
        btn.disabled = true;
        return;
    }
...
function collectPostSurveyMatrix1() {
    ...
    for (const stem of POST_SURVEY_M1_STEMS) {
        ...
        for (const row of POST_SURVEY_M1_ROWS) {
            const sel = document.querySelector(`input[name="post-m1-${stem}-${row}"]:checked`);
            if (!sel) return null;
            out[stem][row] = sel.value;
        }
    }
    return out;
}
...
function collectPostSurveyMatrix2() {
    const out = {};
    for (const k of POST_SURVEY_M2_KEYS) {
        const sel = document.querySelector(`input[name="post-m2-${k}"]:checked`);
        if (!sel) return null;
        out[k] = sel.value;
    }
    return out;
...
function isPostSurveyFormComplete() {
    ...
    const imp = document.querySelector('input[name="post-impressive"]:checked');
    if (!q1 || !q2 || !q3 || !q4 || !l1 || !l2 || !imp) return false;
    if (!collectPostSurveyMatrix1()) return false;
    if (!collectPostSurveyMatrix2()) return false;
    return true;
}
...
function updatePostGameSurveySubmitEnabled() {
    ...
    submit.disabled = !isPostSurveyFormComplete();
}
...
function initPostGameSurveyListeners() {
    ...
    document.getElementById('post-game-survey-form-wrap')?.addEventListener('change', (e) => {
    ...
    document.getElementById('post-game-survey-submit-btn')?.addEventListener('click', () => {
        ...
        if (!isPostSurveyFormComplete() || !matrix1 || !matrix2 || !impressive_if) {
            if (err) {
...
socket.on('game_joined', (data) => {
    ...
    try {
        ...
        if (!gameState.isAdmin && data.player && data.player.post_survey_game_over) {
            renderGameOverContent(data.player.post_survey_game_over);
            showScreen('gameover');
...
socket.on('post_game_survey_saved', (data) => {
    ...
    const payload = data && data.game_over;
    if (payload) {
        renderGameOverContent(payload);
        showScreen('gameover');
    } else {
        const wrap = document.getElementById('post-game-survey-form-wrap');
        const done = document.getElementById('post-game-survey-done');
